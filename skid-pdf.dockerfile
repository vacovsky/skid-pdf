...
RUN yum install -y wkhtmltopdf openssl xorg-x11-server-Xvfb urw-fonts npm
...
RUN mkdir /skidpdf 
ADD src/skid-pdf/skid-pdf /skidpdf
...
RUN cd /skidpdf/static; bower install --allow-root
